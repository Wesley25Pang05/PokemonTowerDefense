...
import { enemies, lowerState, statusPossibility, takeDamage, } from '../round';
import { enemiesGroup } from '../index';

export class Toxel extends Phaser.GameObjects.Image {
  ...
  private shoot(scene: Phaser.Scene, target: Phaser.GameObjects.PathFollower) {
    const projectile = scene.physics.add.image(this.x, this.y, 'projectile12')
      ...
      scene.physics.add.overlap(projectile, enemiesGroup, (proj, enemy) => {
        takeDamage((enemy as any), scene, "Poison", "Special", 21.6, 1/24);
        lowerState(scene, (enemy as any), "specialdef", 0.1);
        projectile.destroy();
        scene.events.off('update', updateHandler);
    });
    const updateHandler = () => {
