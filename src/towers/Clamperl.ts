...
export class Clamperl extends Phaser.GameObjects.Image {
  ...
  constructor(
    ...
    super(scene, x, y, 'pokemon06');
    this.setOrigin(0.5);
    scene.add.existing(this);
    Clamperl.roundsPassed++;
  ...
  private shoot(scene: Phaser.Scene, target: Phaser.GameObjects.PathFollower) {
    const projectile = scene.physics.add.image(this.x, this.y, 'projectile1')
      ...
      scene.physics.add.overlap(projectile, enemiesGroup, (proj, enemy) => {
        takeDamage((enemy as any), scene, "Water", "Special", 17.76, 1/24);
        projectile.destroy();
        scene.events.off('update', updateHandler);
    });
    const updateHandler = () => {
